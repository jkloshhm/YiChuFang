...
public class GridViewWithHeaderAndFooter extends GridView {
    ...
    private int getNumColumnsCompatible() {
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            try {
                ...
                throw new RuntimeException("Can not determine the mNumColumns " +
                        "for this API platform, please call setNumColumns to set it.");
            }
        }
    }
    ...
    private static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
        ...
        public HeaderViewGridAdapter(ArrayList<FixedViewInfo> headerViewInfos,
                                     ArrayList<FixedViewInfo> footViewInfos,
                                     ListAdapter adapter) {
            mAdapter = adapter;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("getView: %s, reused: %s", position,
                        convertView == null));
            }
            // Header (negative positions will throw an ArrayIndexOutOfBoundsException)
            int numHeadersAndPlaceholders = getHeadersCount() * mNumColumns;
        ...
        public int getItemViewType(int position) {
            ...
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("getItemViewType: pos: %s, result: %s", position,
                        type, mCachePlaceHoldView, mCacheFirstHeaderView));
            }
            return type;
        }
